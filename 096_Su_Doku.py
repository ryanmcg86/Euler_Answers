...
Su Doku puzzles ranging in difficulty, but all with unique solutions (the first puzzle in the file is the example above).
...
#Define a sudoku function
def sudoku(s, row = 0):
    if row == 9:
        return s
    elif '0' not in s[row]:
        return sudoku(s, row + 1)
    else:
        col = s[row].index('0')
        for x in '123456789':
            notequal = True
            for i in range(0, 9):
                if s[i][col] == x:
                    notequal = False
            if x not in s[row] and notequal:
                ii = 3 * (row / 3)
                jj = 3 * (col / 3)
                notfound = True
                for i in range(ii, ii + 3):
                    for j in range(jj, jj + 3):
                        if s[i][j] == x:
                            notfound = False
                if notfound:
                    newrow = s[row].replace('0', x, 1)
                    solved = sudoku(s[:row] + [newrow] + s[row + 1:], row)
                    if solved:
                        return solved
...
def solve(filename):
    ...
    #Solve the problem
    unsolved = [lines[i * 10 + 1:i * 10 + 10] for i in range(0, 50)]
    solved = [sudoku(s) for s in unsolved]
    ans = str(sum(int(s[0][:3]) for s in solved))
    
    #Print the results
    print 'The sum of the 3-digit numbers found in the top '
    print 'left corner of each solution grid is ' + ans + '.'
